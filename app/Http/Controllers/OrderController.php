...
use App\Payment;
use App\Ingredient;
use App\Mail\OrderFoodDeliveryBoyMail;
...
class OrderController extends Controller
{
    ...
    public function deleteorder($id)
    {
        ...
        $user = AppUser::find($store->user_id);

        $details = [
            'subject' => 'Message from Indhosnacks.com',
            'greeting' => 'Hi ' . $user->name . ', ',
            'body' => $user->name . ' ' . 'we are sorry to inform you that, Your order has been delete by Indhosnacks.com',
    ...
    public function orderaction($id, $status)
    {
        ...
        if ($status == 1) {
            $statusmassege = "your order has beed accepted by Indhosnacks.com, they are start preparing order. You ordered food will be arrived to you as soon as possible";
        } elseif ($status == 2) {
            $statusmassege = "We are sorry that your order has been rejected by Indhosnacks.com";
        } elseif ($status == 3) {
            $statusmassege = "";
        } elseif ($status == 4) {
            $statusmassege = "We are sorry that your order has been delete by Indhosnacks.com";
        } else {
            $statusmassege = "We are sorry that your order has been rejected by Indhosnacks.com";
        }
    ...
    public function readyforpickup($id, $status)
    {
        ...
        if ($update) {
            ...
            $details = [
                'subject' => 'Message from Indhosnacks.com',
                'greeting' => 'Hi ' . $user->name . ', ',
                'body' => $user->name .' '. 'your order ready to pickup. Please be ready for pick up of your order.',
                'email' => 'Your email is : ' . $user->email,
                'phone' => 'Your phone number is : ' . $user->mob_number,
                'thanks' => 'Thank you for using Indhosnacks',
                'site_url' => route('website.home'),
    ...
    public function waitingforpickup($id)
    {
        ...
        if ($update) {
            ...
            $details = [
                'subject' => 'Message from Indhosnacks.com',
                'greeting' => 'Hi ' . $user->name . ', ',
                'body' => $user->name .' '. 'Your order is Completed. Thank you for order from indhosnacks.com. Stay with Indhosnacks.com',
                'email' => 'Your email is : ' . $user->email,
                'phone' => 'Your phone number is : ' . $user->mob_number,
                'thanks' => 'Thank you for using Indhosnacks',
                'site_url' => route('website.home'),
    ...
    public function assignorder(Request $request)
    {
        ...
        if ($getorder) {
            ...
            $foodDeliveryBoy = DB::table('food_delivery_boy')->where('id', $request->get("assign_id"))->first();

            $details = [
                'subject' => 'Message from Indhosnacks.com',
                'greeting' => 'Hi ' . $foodDeliveryBoy->name . ', ',
                'body' => $foodDeliveryBoy->name .' '. 'You are assign for a food delivery. Plase take your order and give it to the user.',
                'email' => 'Your email is : ' . $foodDeliveryBoy->email,
                'phone' => 'Your phone number is : ' . $foodDeliveryBoy->mobile_no,
                'thanks' => 'Thank you for using Indhosnacks',
                'site_url' => route('website.home'),
                'site_name' => 'Indhosnacks.com',
                'copyright' => 'Copyright © ' . Carbon::now()->format('Y') . ' ' . 'IndhoSnacks. All rights reserved.',
            ...
            Mail::to($foodDeliveryBoy->email)->send(new OrderFoodDeliveryBoyMail($details));
